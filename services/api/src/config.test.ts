...
describe('API environment configuration', () => {
  it('uses the efficient GPT-5.6 model tiers by default', () => {
    ...
    expect(config.openai.sttModel).toBe('gpt-transcribe');
  ...
  it('upgrades the legacy STT model setting so deployed servers preserve spoken language', () => {
    const config = loadApiConfig({ OPENAI_API_KEY: 'test-key', OPENAI_STT_MODEL: 'gpt-4o-transcribe' });
    expect(config.openai.sttModel).toBe('gpt-transcribe');
  });
